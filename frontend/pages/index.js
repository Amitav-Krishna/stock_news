...
export default function Home() {
    ...
    const getChartDataForDate = (articleDate) => {
        ...
        return chartData.find(
            (point) =>
                new Date(point.time).toDateString() ===
                new Date(articleDate).toDateString(),
        );
    };
    const fetchStockData = async () => {
        try {
            ...
            const apiBase =
                process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:3300";

            // Fetch stock data
            const stockResponse = await axios.get(
                `${apiBase}/api/stock/${ticker}`,
            );
            console.log("Stock response:", stockResponse);
            // Fetch news data
            const newsResponse = await axios.get(`${apiBase}/api/news`, {
                params: { q: ticker },
            });

            // Process responses
            const processedData = stockResponse.data.data.map((item) => ({
                ...item,
                price: Number(item.price) || 0,
                hasArticle: newsResponse.data.some(
                    (article) =>
            ...
            if (error.response?.status === 429) {
                ...
                setCachedStocks(error.response.data?.cachedStocks || []);
            }
        }
    };
    const CustomTooltip = ({ active, payload, label, selectedPoint }) => {
        if ((!active && !selectedPoint) || !payload || !payload.length)
            return null;
        ...
        const showTooltip =
            active ||
            (selectedPoint &&
                new Date(selectedPoint).toDateString() ===
        ...
        return (
            ...
            >
                <p>
                    <strong>Date:</strong>{" "}
                    {format(new Date(label), "MM/dd/yyyy")}
                </p>
                <p>
                        ...
                        <div className="mt-8">
                            ...
                            <div className="space-y-4">
                                {articles.map((article, index) => {
                                    ...
                                    const dataPoint = getChartDataForDate(
                                        article.publishedAt,
                                    );
